...
gulp.task('copy:bootstrap', function () {
    gulp.src('./node_modules/bootstrap/dist/**/*').pipe(gulp.dest(config.dist.lib + '/bootstrap'));
    gulp.src('./node_modules/bootstrap/dist/fonts/**/*').pipe(gulp.dest(config.dist.fonts));
});
//handlebars
...
gulp.task('build:less', function () {
    gulp.src(config.path.mainless).pipe(sourcemaps.init()).pipe($.header(banner, {pkg: pkg, ver: ''})).pipe($.plumber({
...
gulp.task('build:js', function () {
    gulp.src(config.path.mainjs).pipe(sourcemaps.init()).pipe(browserify()).pipe($.header(banner, {
        pkg: pkg,
        ver: ''
    })).pipe($.plumber({
        errorHandler: function (err) {
...
gulp.task('copy:lib', function (cb) {
    runSequence(['copy:jquery', 'copy:bootstrap', 'copy:handlebars', 'copy:jslib'], cb);
});
